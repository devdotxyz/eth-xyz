class EthXyzLoader {
  constructor(domain, isLogging) {
    ...
    this.getTextRecords().then((textRecords) => {
  ...
  renderProfile() {
    ...
    if(description === null && email === null && github === null && keybase === null && linkedin === null && peepeth === null && phone === null && telegram === null && twitter === null && url === null && contentHash === null) {
  ...
  renderAvatar() {
    ...
    if(textRecordAvatar !== null && textRecordAvatar !== undefined && textRecordAvatar.indexOf('http') !== -1) {
  ...
  renderNftModal(e) {
    ...
    let creator_username = (nft.creator != null && nft.creator.user != null && nft.creator.user.username != null) ? nft.creator.user.username : null;
    let creator_avatar = (nft.creator != null && nft.creator.profile_img_url != null) ? nft.creator.profile_img_url : null;
    let image_url = '';
    ...
    document.addEventListener("click", function (event) {
      if (!modalContainer.classList.contains('invisible') && (event.target == modalMain || event.target == modalContainer)) {
        modalContainer.classList.remove('visible');
        modalContainer.classList.add('invisible');
      }
    });
